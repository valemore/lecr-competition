...
def get_source_nonsource_topics(corr_df, topics_df):
    ...
    source_topics = sorted([topic_id for topic_id in corr_df["topic_id"] if t2category == "source"])
    nonsource_topics = sorted([topic_id for topic_id in corr_df["topic_id"] if t2category != "source"])
