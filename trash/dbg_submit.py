...
from utils import flatten_positive_negative_content_ids
...
def main(classifier_thresh: float,
    ...
    corr_df = pd.read_csv("../cross/distiluse-base-multilingual-cased-v2_KLB-83.csv", keep_default_na=False)
    ...
    for topics_in_scope_train_idxs, topics_in_scope_val_idxs in KFold(n_splits=5).split(topics_in_scope):
        topic_ids = [topics_in_scope[idx] for idx in topics_in_scope_val_idxs]
        break
